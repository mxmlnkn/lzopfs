...
void FileHandle::throwEx( const char *call,
                          int         err ) const
{
    std::string why( call );
...
void FileHandle::read( Buffer& buf,
                       size_t  size )
{
    buf.resize( size );
    read( &buf[0], size );
...
void FileHandle::pread( off_t  off,
                        void * buf,
                        size_t size ) const
{
...
size_t FileHandle::tryPRead( off_t   off,
                             Buffer& buf,
                             size_t  size ) const
{
    buf.resize( size );
...
size_t FileHandle::tryRead( Buffer& buf,
                            size_t  size )
{
    buf.resize( size );
...
size_t FileHandle::tryRead( void * buf,
                            size_t size )
{
...
void FileHandle::write( const void *buf,
                        size_t      size )
{
...
off_t FileHandle::seek( off_t offset,
                        int   whence )
{
...
void FileHandle::convertBEBuf( char * buf,
                               size_t size )
{
