...
void LzopFile::readHeaders( FileHandle& fh,
                            uint32_t&   flags )
{
    try {
        // Check magic
...
LzopFile::Checksum LzopFile::checksum( ChecksumType  type,
                                       const Buffer& buf )
{
...
off_t LzopFile::findBlocks( FileHandle& fh,
                            uint32_t    flags,
                            off_t       uoff )
{
...
void LzopFile::decompressBlock( const FileHandle& fh,
                                const Block&      b,
                                Buffer&           ubuf ) const
{
...
std::string LzopFile::destName() const
{
    ...
    if ( replaceExtension( base, "tzo", "tar" ) ) {
        return base;
    }
    if ( removeExtension( base, "lzo" ) ) {
        return base;
    }
    return base;
}
